...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <tbody class="divide-y divide-slate-100">
            @forelse ($categorias as $categoria)
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-5 py-3.5 font-medium text-slate-800">{{ $categoria->nombre }}</td>
                    <td class="px-5 py-3.5 text-center text-slate-500">{{ $categoria->productos_count }}</td>
    ...
    @if ($categorias->hasPages())
        <div class="px-5 py-4 border-t border-slate-200">{{ $categorias->links() }}</div>
    @endif
</div>
